function [trl,eventlist] = my_trialfunction_eyetracker_rewardtask_2sess_stim(cfg)

hdr = ft_read_header(cfg.dataset);
event = ft_read_event(cfg.dataset);

%take out 61,70,122,171 trials (only fixation cross : ID 1001) --> adjust this later!
raus = [0 30 60 90 120 150];
% raus = [0];

trlnums = [event.value];
rausrows = find(ismember(trlnums,raus));
rausrows2 = rausrows(1:6:end);
event(rausrows2) = [];
...
for e = 1:5:length(event)
    ...
    event(e+2).value = 3; % dot
    event(e+3).value = 4; % feedback
    event(e+4).value = 5; % fix
end
