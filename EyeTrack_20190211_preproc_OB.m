%% Pupillometric data preprocessing: Novelty Task (Oddball)
...
session     = str2num(answ{1,1}); clear answ
...
path_behav  = [ path_dat 'EAC_behav/pilot_3T/' ];
path_save   = [ path_parent 'EB_cleaned/EBB_pupil/'];

% variables
% VPs = [1204 1107 1208 1109 1210 1111 1212 1113 1214 1216];
VPs = [1204 1107 1208]; % with three segments  - stim / resp / fix
fname_raweye = []; fname_beh =[]; expdat = [];
for i1 = 1:length(VPs)
    fname_raweye{i1,1}  = [ 'ob' num2str(VPs(i1)) '.asc' ]; % oddball
...
pathdat       = fullfile(path_dat,'EAA_pupil','pilot_3T');
...
%         end
    ...
    if expdat{vps,1}.dat.oddball.StimSchedule == 1
        cfg.trialfun        = 'my_trialfunction_eyetracker_OB_1'; % for the second pilot, which didn't have feedbacks
    else
        cfg.trialfun        = 'my_trialfunction_eyetracker_OB_2';
    end
    ...
    cfg.trialdef.eventvalue = [1 2]; % triggers
...
%     if vps <= 3
%         
%         tmp = expdat{i1,1}.dat.oddball.results.presentation;
%         
%         for k = 1:length(tmp)
%             ktmp{k,1} = tmp{k,1}(end);
%         end
%         
%         indstim = find(contains(ktmp,'p')); clear ktmp tmp
%         tempeye = eye_stim;
%         tempeye.time = eye_stim.time(indstim);
%         tempeye.trial = eye_stim.trial(indstim);
%         tempeye.sampleinfo = eye_stim.sampleinfo(indstim,:);
%         tempeye.cfg.trl = eye_stim.cfg.trl(indstim,:);
%         tempeye.cfg.event = eye_stim.cfg.event(indstim);
%         
%         eye_stim = tempeye; clear tempeye indstim
%     else
%     end
    ...
    load([pathdat '/' 'ob' num2str(VPs(vps)) 'stim_eyedat.mat']);
...
%     eye_rv.accuracy = expdat{vps,1}.dat.oddball.results.accuracy(trl_raus==0);
%     eye_rv.keypress = expdat{vps,1}.dat.oddball.results.keypress(trl_raus==0);
%     eye_rv.rts      = expdat{vps,1}.dat.oddball.results.rt(trl_raus==0);
%     eye_rv.trlord   = expdat{vps,1}.dat.oddball.results.trl(trl_raus==0,:);
    eye_rv.presentation = expdat{vps,1}.dat.oddball.results.presentation(trl_raus==0,:);
    ...
    save([path_save 'pilot_3T/ob' num2str(VPs(vps)) '_eye_rv200_stim.mat'],'eye_rv','blinkbig','blinksmall')
