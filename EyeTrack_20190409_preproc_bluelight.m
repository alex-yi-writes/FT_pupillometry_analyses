...
% variables
VPs = [3203 3205 3206 3208 3210 3211 3212 3213 3214 3215 3216]; % 3214 bad data
% VPs = [3213 3214 3215 3216]; % exclude 3209 for bad data
% VPs = 3203;
...
for vps = 1%:length(VPs)
    ...
    % stimulus segment
    cfg                     = [];
    cfg.trialfun            = 'my_trialfunction_eyetracker_bluelight_maintask_stim'; % picking stimulus screen for now
    ...
    % cue segment
    cfg                     = [];
    cfg.trialfun            = 'my_trialfunction_eyetracker_bluelight_maintask_cue_short';
    cfg.dataset             = [pathdat '/' name]; % ascii converted eyelink filename
    cfg.headerformat        = 'eyelink_asc';
    cfg.dataformat          = 'eyelink_asc';
    cfg.trialdef.eventtype  = 'MSG';
    ...
    save([pathdat '/' name(1:end-4) 'cue_eyedat.mat'],'eye_cue');
    fname_fbSeg = [pathdat '/' name(1:end-4) 'cue_eyedat.mat'];
    
    data_cue = cell2mat(eye_cue.trial');
    figure,errorbar(nanmedian(data_cue),nanstd(data_cue)./sqrt(size(data_cue,1))),title(name);
    h = gcf;
    saveas(h,[pathdat '/' name(1:end-4) 'cue_eyemedian'],'fig')
    figure,plot(nanmedian(data_cue)),title(name);
    ...
    load([pathdat '/' 'be' num2str(VPs(vps)) 'stim_eyedat.mat']);
...
%     end
    ...
    eye_rv.accuracy = expdat{vps,1}.dat.bluelight.task.results.accuracy(trl_raus==0);
    eye_rv.keypress = expdat{vps,1}.dat.bluelight.task.results.keypress(trl_raus==0);
    eye_rv.rt_resp  = expdat{vps,1}.dat.bluelight.task.results.rt_resp(trl_raus==0);
    eye_rv.trlord   = expdat{vps,1}.dat.bluelight.task.results.trl(trl_raus==0,:);
    ...
    save([path_save '/be' num2str(VPs(vps)) '_eye_rv200_stim.mat'],'eye_rv')
    ...
    fprintf('\n cleaning (cue segment) ID: %d\n', VPs(vps))
    
    datacall = []; dataiall = []; trls = []; samples = [];
    
    load([pathdat '/' 'be' num2str(VPs(vps)) 'cue_eyedat.mat']);
    
    data_cue    = cell2mat(eye_cue.trial');
    samples = [samples;eye_cue.sampleinfo+1*1*10^6];%%% newly added, should be done with all future datasets
    trls    = [trls;eye_cue.cfg.trl+1*1*10^6];%%% newly added, should be done with all future datasets
    
    %  exclude data parts with closed eyes
    for tr = 1:size(data_cue,1)
        ...
        if ~isempty(emptySa)
            ...
            datac(tr,1:size(data_cue,2)) = temp(1:size(data_cue,2));
            blinkbig(tr) = sum(isnan(temp)); % calculate how many ms exclued because of blinks
            ...
            temp(zs) = []; x(zs)=[];
            temp2   = interp1(x, temp, xi);
            datai(tr,1:size(data_cue,2)) = temp2(1:size(data_cue,2));
...
%     figure,
%     for i = 1:80
%         subplot(10,8,i);plot(data_cue(i,:));hold on
%         plot(datai(i,:),'r');hold off
%     end
    datacall = [datacall; datac]; clear datac
    dataiall = [dataiall; datai]; clear datai
    ...
    for tr = 1: size(dataiall,1)
        ...
        temp    = zscore(dataiall(tr,:));
        basel   = nanmean(temp(1:200)); % 200 ms before cue onset
        eyeinterp.trial{1,tr}   = temp-basel; clear temp
        eyeinterp.time{1,tr}    = eye_cue.time{1};
    ...
    save([path_save '/be' num2str(VPs(vps)) '_eye_rv200_cue.mat'],'eye_rv')
    %     keep cutleft cutright pathdat pathdatbehav fenster smallblinksize id VPs session expdat i2
    close all
