function [trl,eventlist] = my_trialfunction_eyetracker_continuous_alex(cfg)

hdr = ft_read_header(cfg.dataset);
event = ft_read_event(cfg.dataset);

% get time of begin of task (MSG -100, which is start of first volume andp
% start segmenting 1sec after that, also mark end of recording (MSG - 200)

trl = [];eventlist = [];

last_trigger = event(end).sample;
first_trigger = event(2).sample - 3600*4;
